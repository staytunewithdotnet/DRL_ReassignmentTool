...
{
    public class AVPService : IAVPService
    {
        private readonly IAVPMasterRepository _avpRepository;
        private readonly IUnitOfWork _unitofwork;
        private readonly ILogger logger;
        private readonly CommonHelper CommonHelper;

        public AVPService(IUnitOfWork unitofwork, IAVPMasterRepository avpRepository, ILogManager logManager)
        {
            _avpRepository = avpRepository;
            _unitofwork = unitofwork;
            logger = logManager.GetLogger(this.GetType());
            CommonHelper = new CommonHelper();
        }
        public List<ENTLookUpItem> GetAVPsLookup()
        {
            List<ENTLookUpItem> result = new List<ENTLookUpItem>();
            try
            {
                result = _avpRepository.GetAllAVPs().Select(p => Configuration.Mapper.Map<ENTLookUpItem>(p)).ToList();
            }
            catch (Exception ex)
            {
